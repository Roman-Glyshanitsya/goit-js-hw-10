...
function handleSearch(e) {
  ...
  fetchCountries(inputValue)
    .then(fetchResponse)
    .catch(error => Notiflix.Notify.failure(`Something went wrong - ${error}`));
}
...
function fetchResponse(countries) {
  ...
  if ((countries.length > 1) & (countries.length <= 10)) {
    ...
    const countryInfo = countries
      .map(country => {
        return `<li class = "country-item-info"><div class="top"><img src='${
          country.flags.svg
        }', width=4%, height=4%, alt='flag'>
      ...
      <p class="header">Languages: <span class="text">${Object.values(
        country.languages,
      )}</span></p></li>`;
      })
      .join('');
